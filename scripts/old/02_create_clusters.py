...
clusters = []
path = "./subfamilies/"
for infile in glob.glob( os.path.join(path, '*.clstr') ):
    clusters.append(infile)
...
result = proteome.find_proteins_by("cluster",
                                   ["bZIP","zf-C2H2","Homeobox",
                                    "other","NuclearFactors","HLH","P53"])

os.mkdir("cluster_subfamilies")
os.chdir("cluster_subfamilies")

for family, clusters in result.items():
    os.mkdir(family)
    os.chdir(family)
    for cluster in clusters:
        subfamily_file = open(cluster.name + ".fasta","w")
        for member in cluster.members:
            fasta = member.export_fasta()
            subfamily_file.write(fasta[0])
            subfamily_file.write(fasta[1].replace("*",""))
        subfamily_file.close()
    os.chdir("../")
